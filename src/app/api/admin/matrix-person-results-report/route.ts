...
export async function POST(req: NextRequest) {
  ...
  const rl = await rateLimitByUser(req, 'admin:matrix-person-results-report:post', String(s.uid || ''), 10, 60 * 1000)
  if (rl.blocked) {
  ...
  try {
    const report = await buildMatrixPersonResultsReport(supabase, {
      periodId,
      orgId,
      lang: safeLang,
      department,
    ...
    const msg = e instanceof Error ? e.message : 'Kişi bazlı MATRIX raporu alınamadı'
